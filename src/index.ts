...
export const Config: Schema<Config> = Schema.intersect([
  Schema.object({
    allowZeroBetJoin: Schema.boolean()
      .default(true)
      .description(`是否开启零投注也能加入游戏的功能。`),
    enableCardBetting: Schema.boolean()
      .default(false)
      .description(`是否开启投注牌型功能。`),
    enableSurrender: Schema.boolean()
      .default(false)
      .description(`是否开启投降功能。`),
  }).description("一般设置"),
  ...
  Schema.object({
    ...
    isTextToImageConversionEnabled: Schema.boolean()
      .default(false)
      .description(
        `（QQ 官方机器人必须开启，防违规检测）是否开启将文本转为图片的功能（可选），如需启用，需要启用 \`markdownToImage\` 服务。`
      ),
    isEnableQQOfficialRobotMarkdownTemplate: Schema.boolean()
      .default(false)
      .description(
        `（QQ 官方机器人必须开启文本转图片功能，用于防违规检测）是否启用 QQ 官方机器人的 Markdown 模板，带消息按钮。`
      ),
  }).description("消息处理设置"),
  Schema.union([
    Schema.object({
      ...
      key: Schema.string()
        .default("")
        .description(
          `文本内容中特定插值的 key。如果你的插值为 {{.info}}，那么请在这里填 info。`
        ),
  ...
  Schema.object({
    minimumRequiredCurrencyForGameEntry: Schema.number()
      .min(0)
      .default(0)
      .description(`加入游戏所需的最低货币数量。`),
    dealerSpeed: Schema.number()
      .min(0)
      .default(2)
      .description(`庄家要牌的速度，单位是秒。`),
    betMaxDuration: Schema.number()
      .min(0)
      .default(30)
      .description(`投注牌型操作的等待时长，单位是秒。`),
    buyInsuranceMaxDuration: Schema.number()
      .min(0)
      .default(10)
      .description(`买保险操作的等待时长，单位是秒。`),
    surrenderMaxDuration: Schema.number()
      .min(0)
      .default(10)
      .description(`投降操作的等待时长，单位是秒。`),
    joinGameProcedureWaitTimeInSeconds: Schema.number()
      .min(0)
      .default(2)
      .description(`办理加入游戏手续等待时间，单位是秒。`),
  }).description("游戏操作设置"),
  ...
  Schema.object({
    numberOfDecks: Schema.number()
      ...
      .default(4)
      .description(
        `使用几副扑克牌，默认为 4 副（因为闲家都是明牌，所以建议使用默认值）。`
      ),
  }).description("扑克牌设置"),
  ...
  Schema.object({
    transferFeeRate: Schema.number()
      .default(0.1)
      .description(`转账收取的手续费比例。`),
    isBellaPluginPointsEnabledForCurrency: Schema.boolean()
      .default(false)
      .description(`是否启用 Bella 签到插件的积分作为货币。`),
  }).description("费用设置"),
]) as any;
...
export function apply(ctx: Context, config: Config) {
  ...
  ctx
    .command("blackJack", "blackJack/21点游戏帮助")
...
`,
  ...
  // zz*
  ctx
    .command("blackJack.转账 [content:text]", "转账")
    .userFields(["id", "name", "permissions"])
...
> @小小学`;
      ...
      if (
        ...
        targetPlayerRecord = await ctx.database.get("blackjack_player_record", {
          userId,
        });
        if (targetPlayerRecord.length === 0) {
          targetPlayerRecord = await ctx.database.get(
            "blackjack_player_record",
            { username }
          );
          if (targetPlayerRecord.length === 0) {
        ...
        const userIdRegex =
          /<at id="(?<userId>[^"]+)"(?: name="(?<username>[^"]+)")?\/>/;
        const match = content && content.match(userIdRegex);

        if (!match) {
          const contentArray = content.split(" ");
          userId = contentArray[0];
          const targetPlayerRecord: BlackJackPlayerRecord[] =
            await ctx.database.get("blackjack_player_record", { userId });
          if (targetPlayerRecord.length === 0) {
            return await sendMessage(
              session,
              `【@${sessionUserName}】\n未找到符合要求的用户 ID。`,
      ...
      if (config.isBellaPluginPointsEnabledForCurrency) {
        const bellaSignIn: BellaSignIn[] = await ctx.database.get(
          "bella_sign_in",
          { id: session.userId }
        );
...
而转账后您仅剩：【${userMoney - amount - transferFee}】`,
      ...
      if (config.isBellaPluginPointsEnabledForCurrency) {
        const targetUser = await ctx.database.get("bella_sign_in", {
          id: userId,
        });
        ...
        const getUserMonetary2 = await ctx.database.get("monetary", {
          uid: uid2,
        });
        ...
        if (getUserMonetary2.length === 0) {
          ...
          await ctx.database.set(
            "monetary",
            { uid: uid2 },
            { value: userMonetary2.value + amount }
          );
        }
      }
...
您的余额为：【${newScore}】`,
  ...
  // 加入游戏并投注筹码
  ctx
    .command("blackJack.加入游戏 [bet:number]", "加入游戏并投注筹码")
    .userFields(["id", "name", "permissions"])
    .action(async ({ session }, bet) => {
      ...
      // 玩家记录表操作
      const userRecord = await ctx.database.get("blackjack_player_record", {
        userId,
      });
      ...
      // 查询当前群组的游戏记录
      let gameRecord = await ctx.database.get("blackjack_game_record", {
        channelId,
      });

      // 如果当前群组没有游戏信息，则新建一个
      if (gameRecord.length === 0) {
        await ctx.database.create("blackjack_game_record", {
          channelId,
          gameStatus: "未开始",
        });
        gameRecord = await ctx.database.get("blackjack_game_record", {
          channelId,
        });
      }
...
买定离手，无法再更改投注！`,
      ...
      if (!bet) {
        let userMoney = 0;
        if (config.isBellaPluginPointsEnabledForCurrency) {
          const bellaSignIn = await ctx.database.get("bella_sign_in", {
            id: session.userId,
          });
          if (bellaSignIn.length === 0) {
            if (!allowZeroBetJoin) {
...
已经为您办理货币登记了呢~`,
        ...
        const isBalanceSufficient = allowZeroBetJoin
          ? userMoney < 0
          : userMoney <= 0;
...
您的货币余额为：【${userMoney}】
${
  allowZeroBetJoin && userMoney === 0
    ? "检测到允许零投注！\n正在为您办理加入游戏手续中..."
    : "请输入您的【投注金额】："
}`,
          `${allowZeroBetJoin && userMoney === 0 ? "" : `输入投注金额`}`
        );
      ...
      // 最少所需投注
      if (
        config.minimumRequiredCurrencyForGameEntry > 0 &&
        bet < config.minimumRequiredCurrencyForGameEntry
      ) {
        return await sendMessage(
          session,
          `【@${username}】\n您的投注不够呢！\n最少所需投注为：【${config.minimumRequiredCurrencyForGameEntry}】`,
          `改名 无庄模式 开始游戏 退出游戏 加入游戏 转账`
        );
      }
      // 检查是否存在有效的投注金额
...
不允许零投注哦~`,
      ...
      const [playerRecord] = await ctx.database.get("blackjack_player_record", {
        userId,
      });
      await ctx.database.set(
        "blackjack_player_record",
        { userId },
        { moneyChange: playerRecord.moneyChange - bet }
      );
      ...
      const numberOfPlayers = (
        await ctx.database.get("blackjack_playing_record", { channelId })
      ).length;
...
${!isBalanceSufficient ? "检测到余额不足！\n已自动向下合并！\n\n" : ""}${
          bet === 0 && allowZeroBetJoin && userMoney !== 0
            ? "检测到允许零投注！\n\n"
            : ""
        }投注成功！
您正式加入游戏了！
...
当前玩家人数：${numberOfPlayers} 名！`,
  ...
  ctx
    ...
    .action(async ({ session }) => {
      ...
      username = await getSessionUserName(session);
      // 检查游戏状态
      const gameInfo = await ctx.database.get("blackjack_game_record", {
        channelId,
      });
      ...
      // 检查玩家是否加入游戏
      const playerInfo = await ctx.database.get("blackjack_playing_record", {
        channelId,
        userId,
      });
      ...
      if (config.isBellaPluginPointsEnabledForCurrency) {
        ...
        if (bellaSignIn.length !== 0) {
          await ctx.database.set(
            "bella_sign_in",
            { id: player.userId },
            { point: bellaSignIn[0].point + player.bet }
          );
        }
      } else {
        // @ts-ignore
      ...
      // 从游戏中移除玩家
      await ctx.database.remove("blackjack_playing_record", {
        channelId,
        userId,
      });
...
剩余玩家人数：${numberOfPlayers} 名！`,
  ...
  ctx
    ...
    .action(async ({ session, options }) => {
      ...
      const getPlayers: BlackJackPlayingRecord[] = await ctx.database.get(
        "blackjack_playing_record",
        { channelId }
      );
        ...
        if (options.noDealerMode) {
          ...
          for (const player of getPlayers) {
            if (player.bet > minBet) {
              ...
              if (config.isBellaPluginPointsEnabledForCurrency) {
                ...
                if (bellaSignIn.length !== 0) {
                  await ctx.database.set(
                    "bella_sign_in",
                    { id: player.userId },
                    { point: bellaSignIn[0].point + refundAmount }
                  );
                }
              } else {
                const uid = (
                  await ctx.database.getUser(platform, player.userId)
                ).id;
              ...
              await ctx.database.set(
                "blackjack_playing_record",
                { channelId, userId: player.userId },
                { bet: minBet }
              );
            }
          }
          ...
          let playerWithIndexOne: {
            player: Pick<
              BlackJackPlayingRecord,
              Keys<BlackJackPlayingRecord, any>
            >;
          };
          let betPlayer: Pick<
            BlackJackPlayingRecord,
          ...
          >;
          playerWithIndexOne =
            shuffledPlayersWithIndex.length > 0
              ? shuffledPlayersWithIndex.find((item) => item.index === 1)
              : null;
...
【要牌】或【停牌】`,
          ...
          await ctx.database.set(
            ...
            { channelId },
            {
              ...
              currentPlayerUserId: betPlayer.userId,
              currentPlayerUserName: betPlayer.username,
              gameStatus: "已开始",
        ...
        const playerOrder = shuffledPlayersWithIndex
          .map(
            (item) => `${item.index}. 【${item.player.username ?? "Unknown"}】`
          )
          .join("\n");
...
${
  !enableCardBetting || !enableSurrender
    ? `正在为庄家发牌...\n\n请庄家亮牌！`
    : ""
}`,
          `${enableCardBetting ? `跳过投注 投注牌型` : ""}`
        );
      } else if (numberOfPlayers === 1) {
        const player = getPlayers[0];
        await ctx.database.set(
          "blackjack_playing_record",
          { userId: player.userId, channelId },
          {
            playerIndex: 1,
            playerHandIndex: 1,
          }
...
${
  !enableCardBetting || !enableSurrender
    ...
    : ""
      ...
      const shuffledDeck = shuffleArrayMultipleTimes(decks, numTimes);
      let playerWithIndexOne: {
        player: Pick<BlackJackPlayingRecord, Keys<BlackJackPlayingRecord, any>>;
      };
      let betPlayer: Pick<
        BlackJackPlayingRecord,
        Keys<BlackJackPlayingRecord, any>
...
【跳过投降】：直接进入下一阶段。`,
        ...
        // 判断游戏在投降之后是否已经结束
        const result = await ctx.database.get("blackjack_game_record", {
          channelId,
        });
        if (result.length === 0) {
          return;
...
【要牌】或【停牌】`,
  ...
  ctx
    ...
    .action(async ({ session }) => {
      ...
      // 检查游戏信息是否存在
      const getGameInfo = await ctx.database.get("blackjack_game_record", {
        channelId,
      });
...
您当前的余额为：【${userMonetary.value}】`,
  ...
  ctx
    ...
    .action(async ({ session }) => {
      ...
      if (gameInfo.gameStatus === "未开始") {
        // 退钱
        const getPlayers = await ctx.database.get("blackjack_playing_record", {
          channelId,
        });
        for (const player of getPlayers) {
  ...
  ctx
    .command(
      "blackJack.投注 [playerIndex:number] [betType:string] [betAmount:number]",
      "投注牌型"
    )
    .userFields(["id", "name", "permissions"])
...
示例：投注 1 7 50`,
      ...
      const numberOfPlayers = getPlayers.length;
      const getThisPlayer = await ctx.database.get("blackjack_playing_record", {
        channelId,
        userId,
      });
      ...
      if (numberOfPlayers === 1) {
        ...
        const getBetPlayer = await ctx.database.get(
          "blackjack_playing_record",
          { channelId, playerIndex }
        );
      ...
      if (config.isBellaPluginPointsEnabledForCurrency) {
        ...
        if (bellaSignIn.length !== 0) {
          await ctx.database.set(
            "bella_sign_in",
            { id: userId },
            { point: bellaSignIn[0].point - betAmount }
          );
        }
      } else {
...
投注金额：【${betAmount}】`,
  ...
  // tg*
  ctx
    .command("blackJack.跳过投注", "跳过投注牌型的等待时间")
    .userFields(["id", "name", "permissions"])
    .action(async ({ session }) => {
      const sessionUserName = await getSessionUserName(session);
      if (!enableCardBetting) {
      ...
      return await sendMessage(
        session,
        `玩家【@${username}】执行【跳过投注】操作，【投注】通道已关闭！`,
        ``
      );
    });
  // tgmbx*
  ctx
    .command("blackJack.跳过买保险", "跳过买保险的等待时间")
    .userFields(["id", "name", "permissions"])
    .action(async ({ session }) => {
      const sessionUserName = await getSessionUserName(session);
      let { channelId, userId, username, user } = session;
...
【买保险】通道已关闭！`,
  ...
  // tgtx*
  ctx
    .command("blackJack.跳过投降", "跳过投降的等待时间")
    .userFields(["id", "name", "permissions"])
    .action(async ({ session }) => {
      const sessionUserName = await getSessionUserName(session);
      if (!enableSurrender) {
  ...
  ctx
    ...
    .action(async ({ session }) => {
      ...
      const getPlayerInfo = await ctx.database.get("blackjack_playing_record", {
        channelId,
        userId,
        playerIndex: gameInfo.currentPlayerIndex,
      ...
      if (
        getPlayer.length > 1 &&
        calculateScore(player.playerHand[0]) === 11 &&
...
祝你们好运！`,
        ...
        const [newGameInfo] = await ctx.database.get("blackjack_game_record", {
          channelId,
        });
        const [newThisPlayerInfo] = await ctx.database.get(
          "blackjack_playing_record",
          {
            channelId,
        ...
        const distributional = `${
          newThisPlayerInfo.playerHand.length === 1
            ? "但幸运的是，您还有机会！"
            : ""
        }
您的手牌为：【${newThisPlayerInfo.playerHand.join("")}】
请选择您的操作：
${newThisPlayerInfo.playerHand.length === 1 ? "【要牌】🃏" : ""}${
          isHandPair && !gameInfo.isNoDealerMode ? "\n【分牌】👥" : ""
        }${
          score === 11 && playerHand.length === 2 && !gameInfo.isNoDealerMode
            ? "\n【加倍】💰"
            : ""
        }
【停牌】🛑
...
${
  score === 11 && playerHand.length === 2 && !gameInfo.isNoDealerMode
    ...
    : ""
        ...
        const noDistributional = `有请下一位玩家：【@${
...
请选择您的操作：
【要牌】或【停牌】${
          isHandPair && !gameInfo.isNoDealerMode ? "或【分牌】" : ""
        }${
          score === 11 && playerHand.length === 2 && !gameInfo.isNoDealerMode
            ? "或【加倍】"
            : ""
...
${newThisPlayerInfo.playerHandIndex > 1 ? distributional : noDistributional}`;
        return await sendMessage(
          session,
          message,
          `${message.includes("加倍") ? `加倍 ` : ``}${
            message.includes("分牌") ? `分牌 ` : ``
          }要牌 停牌`
        );
      }
      // 未爆牌：
...
您的${
        newThisPlayerInfo.playerHand.length === 1 ? "下一套" : ""
      }手牌为：【${newThisPlayerInfo.playerHand.join("")}】
请选择您的操作：
【要牌】或【停牌】`;
...
下次记得留钱分牌哦！😉`,
      ...
      await ctx.database.create("blackjack_playing_record", {
        ...
        username,
        playerHandIndex: player.playerHandIndex + 1,
      });
      return await sendMessage(
        session,
        `当前玩家是：【@${username}】
...
【要牌】：加倍后只能再要一张牌哦~`,
  ...
  // r* phb*
  ctx
    .command("blackJack.排行榜 [number:number]", "查看排行榜相关指令")
    .userFields(["id", "name", "permissions"])
    .action(async ({ session }, number = defaultMaxLeaderboardEntries) => {
      if (typeof number !== "number" || isNaN(number) || number < 0) {
        return await sendMessage(
          session,
          `请输入大于等于 0 的数字作为排行榜的参数。`,
          `排行榜`
        );
      }
      const leaderboards = {
...
请输入想要查看的【排行榜名】或【序号】：`,
  ...
  ctx
    .command("blackJack.排行榜.胜场 [number:number]", "查看玩家胜场排行榜")
    .userFields(["id", "name", "permissions"])
    .action(async ({ session }, number = defaultMaxLeaderboardEntries) => {
      if (typeof number !== "number" || isNaN(number) || number < 0) {
        return await sendMessage(
  ...
  ctx
    .command("blackJack.排行榜.输场 [number:number]", "查看玩家输场排行榜")
    .userFields(["id", "name", "permissions"])
    .action(async ({ session }, number = defaultMaxLeaderboardEntries) => {
      if (typeof number !== "number" || isNaN(number) || number < 0) {
        return await sendMessage(
  ...
  ctx
    .command("blackJack.排行榜.损益 [number:number]", "查看玩家损益排行榜")
    .userFields(["id", "name", "permissions"])
    .action(async ({ session }, number = defaultMaxLeaderboardEntries) => {
      if (typeof number !== "number" || isNaN(number) || number < 0) {
        return await sendMessage(
  ...
  ctx
    .command(
      "blackJack.排行榜.平局场次 [number:number]",
      "查看玩家平局场次排行榜"
    )
    .userFields(["id", "name", "permissions"])
    .action(async ({ session }, number = defaultMaxLeaderboardEntries) => {
  ...
  ctx
    .command(
      "blackJack.排行榜.21点次数 [number:number]",
      "查看玩家21点次数排行榜"
    )
    .userFields(["id", "name", "permissions"])
    .action(async ({ session }, number = defaultMaxLeaderboardEntries) => {
  ...
  ctx
    .command(
      "blackJack.排行榜.黑杰克次数 [number:number]",
      "查看玩家黑杰克次数排行榜"
    )
    .userFields(["id", "name", "permissions"])
    .action(async ({ session }, number = defaultMaxLeaderboardEntries) => {
  ...
  // cx*
  ctx
    .command("blackJack.查询玩家记录 [targetUser:text]", "查询玩家记录")
    .userFields(["id", "name", "permissions"])
    .action(async ({ session }, targetUser) => {
      ...
      if (!targetUser) {
        targetUserRecord = await ctx.database.get("blackjack_player_record", {
          userId,
        });
      } else {
        ...
        if (
          ...
          if (targetUserRecord.length === 0) {
            targetUserRecord = await ctx.database.get(
              "blackjack_player_record",
              { userId: targetUser }
            );
          }
      ...
      const { win, lose, moneyChange, numberOf21, numberOfBlackJack, draw } =
        targetUserRecord[0];
      return sendMessage(
        session,
...
`,
  ...
  // gm*
  ctx
    .command("blackJack.改名 [newPlayerName:text]", "更改玩家名字")
    .userFields(["id", "name", "permissions"])
  ...
  async function getSessionUserName(session: any): Promise<string> {
    ...
    if (
      ...
      let userRecord = await ctx.database.get("blackjack_player_record", {
      ...
      if (userRecord.length === 0) {
        ...
        userRecord = await ctx.database.get("blackjack_player_record", {
          userId: session.userId,
        });
      }
...
${bankerScore > 21 ? "💥 庄家爆掉了！" : ""}${
        bankerHand.length === 2 && bankerScore === 21
          ? "🎴 庄家黑杰克！"
          : bankerScore === 21
          ? "🎊 庄家21点！"
          : ""
  ...
  async function getLeaderboard(
    ...
    const getPlayers: BlackJackPlayerRecord[] = await ctx.database.get(
      "blackjack_player_record",
      {}
    );
  ...
  async function settleBlackjackGameInNoDealerMode(platform, channelId) {
    const getPlayerRecords: BlackJackPlayingRecord[] = await ctx.database.get(
      "blackjack_playing_record",
      { channelId }
    );
    ...
    for (const record of getPlayerRecords) {
      ...
      if (score === 21) {
        if (playerHand.length === 2) {
          ...
          if (
            (!hasBlackjackWinner && winners.length === 0) ||
            (!hasBlackjackWinner &&
              score > calculateHandScore(winners[0].playerHand))
          ) {
            winners = [record];
          } else if (
            !hasBlackjackWinner &&
    ...
    for (const record of winners) {
      ...
      if (config.isBellaPluginPointsEnabledForCurrency) {
        ...
        if (bellaSignIn.length !== 0) {
          await ctx.database.set(
            "bella_sign_in",
            { id: userId },
            { point: bellaSignIn[0].point + reward }
          );
        }
      } else {
      ...
      await ctx.database.set(
        "blackjack_player_record",
        { userId },
        { moneyChange: playerRecord.moneyChange + reward }
      );
    ...
    const getLosingPlayers = (
      allPlayers: BlackJackPlayingRecord[],
      winningPlayers: BlackJackPlayingRecord[]
    ): BlackJackPlayingRecord[] => {
      return allPlayers.filter(
        (player) =>
          !winningPlayers.some((winner) => winner.userId === player.userId)
      );
    };
  ...
  async function settleBlackjackGame(platform, channelId) {
    const getGameRecords = await ctx.database.get("blackjack_game_record", {
      channelId,
    });
    ...
    // 庄家爆
    if (bankerScore > 21) {
      const getPlayerRecords = await ctx.database.get(
        "blackjack_playing_record",
        { channelId }
      );
      ...
      for (const record of getPlayerRecords) {
        ...
        await ctx.database.set(
          "blackjack_playing_record",
          { channelId, userId, playerHandIndex },
          updateData
        );
      }
    } else {
    ...
    for (const record of getPlayerRecords) {
      const { playerHand } = record;
      let { channelId, userId, playerHandIndex, insurance, isBuyInsurance } =
        record;
    ...
    for (const record of getPlayerRecords) {
      ...
      // 如果投注类型是对子
      if (betType === "对子") {
        const getBetPlayers = await ctx.database.get(
          "blackjack_playing_record",
          { channelId, userId }
        );
    ...
    // 开始结算加钱 暂时不生成最终字符串
    const getThisGuildPlayers = await ctx.database.get(
      "blackjack_playing_record",
      { channelId }
    );
    for (const thisGuildPlayer of getThisGuildPlayers) {
      const { isSurrender, win, bet, insurance, betWin, userId } =
        thisGuildPlayer;
      if (!isSurrender) {
    ...
    async function getSettlementInfo(channelId: string) {
      const players = getThisGuildPlayers.filter(
        (player) => player.channelId === channelId
      );
      ...
      const settlementInfoPromises = Object.keys(mergedRecords).map(
        async (key) => {
          ...
          const usernames = [
            ...new Set(
              players
                .filter(
                  (player) =>
                    player.channelId === channelId && player.userId === userId
                )
          ...
          await ctx.database.set(
            "blackjack_player_record",
            { userId },
            { moneyChange: playerRecord.moneyChange + settlement }
          );
          return usernames.map(
            (username) => `【${username}】: 【${settlement}】`
          );
        }
      );
  ...
  async function isGameEnded(channelId: string): Promise<boolean> {
    const playingRecords = await ctx.database.get("blackjack_playing_record", {
      channelId,
    });
    ...
    const sortedPlayers = filteredPlayers.sort(
      (a, b) =>
        a.playerIndex - b.playerIndex || a.playerHandIndex - b.playerHandIndex
    );
  ...
  function isPair(playerHand: string[]): boolean {
    return (
      playerHand.length === 2 &&
      playerHand[0].slice(1) === playerHand[1].slice(1)
    );
  }
  ...
  function isFlushPair(playerHand: string[]): boolean {
    if (playerHand.length >= 2) {
      ...
      const secondCard = playerHand[1];
      return (
        hasSameValue(firstCard, secondCard) &&
        hasSameSuit(firstCard, secondCard)
      );
    }
    return false;
  }
  ...
  function isSameSuitCombination(
    playerHand: string[],
    bankerHand: string[]
  ): boolean {
    if (bankerHand.length >= 1 && playerHand.length >= 2) {
      ...
      return (
        hasSameSuit(firstBankerCard, firstPlayerCard) &&
        hasSameSuit(firstBankerCard, secondPlayerCard)
      );
    }
    return false;
  }

  function isStraightCombination(
    playerHand: string[],
    bankerHand: string[]
  ): boolean {
    if (bankerHand.length >= 1 && playerHand.length >= 2) {
  ...
  function isSequential(card1: string, card2: string, card3: string): boolean {
    ...
    return (
      sortedValues[0] === sortedValues[1] - 1 &&
      sortedValues[1] === sortedValues[2] - 1
    );
  }
  ...
  function isThreeOfAKind(playerHand: string[], bankerHand: string[]): boolean {
    if (bankerHand.length >= 1 && playerHand.length >= 2) {
      ...
      return (
        firstBankerCard === firstPlayerCard &&
        firstBankerCard === secondPlayerCard
      );
    }
    return false;
  }
  ...
  function hasStraightFlush(
    ...
    return (
      suit1 === suit2 && suit2 === suit3 && isSequential(card1, card2, card3)
    );
  }

  function isFlushThreeOfAKind(
    playerHand: string[],
    bankerHand: string[]
  ): boolean {
    if (bankerHand.length >= 1 && playerHand.length >= 2) {
    ...
    if (
      ...
      buttons.forEach((button, index) => {
        ...
        if (
          ...
          row.buttons.length === config.numberOfMessageButtonsPerRow
        ) {
          rows.push(row);
          row = { buttons: [] };
        }
